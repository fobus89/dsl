...
func TestAnyReturnsMatchedMap(t *testing.T) {
	p := newBinaryTestParser(`r = sample any target`)
...
func TestAllReturnsMatchedMap(t *testing.T) {
	p := newBinaryTestParser(`r = sample all target`)
	p.Ctx().SetValue("sample", value.NewType(map[string]any{
		"id":   1,
		"name": "user",
	}))
...
func TestAnyPrimitiveReturnsMatchedValue(t *testing.T) {
	p := newBinaryTestParser(`r = needle any nums`)
...
func TestAllPrimitiveReturnsMatchedValue(t *testing.T) {
	p := newBinaryTestParser(`r = needle all nums`)
	p.Ctx().SetValue("needle", value.NewType(2))
	p.Ctx().SetValue("nums", value.NewType([]int{1, 2, 3}))
